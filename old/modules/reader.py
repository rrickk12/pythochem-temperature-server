# modules/reader.py
import logging
from db_ops.db_handler import DatabaseManager

logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)

class DataReader:
    """
    @description: Provides utility methods for reading sensor data from the database,
                  performing aggregations and filtering.
    """
    def __init__(self, db_manager: DatabaseManager):
        self.db_manager = db_manager

    def get_raw_data(self, mac, limit=100):
        """
        @description: Retrieves the latest raw readings for a sensor.
        @output: List of raw reading objects.
        """
        data = self.db_manager.get_latest_raw_reads(mac, limit)
        logger.debug("Retrieved raw data for sensor %s: %s", mac, data)
        return data

    def get_statistics(self, mac, start_timestamp, end_timestamp):
        """
        @description: Computes aggregated statistics (mean, min, max) for sensor data.
        @output: Dictionary with statistical data.
        """
        stats = self.db_manager.compute_statistics(mac, start_timestamp, end_timestamp)
        logger.debug("Computed statistics for sensor %s: %s", mac, stats)
        return stats
