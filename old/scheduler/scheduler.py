...
from db_ops.ORM_models import ReadRaw
...
class SchedulerManager:
    """
    @description: Orchestrates scheduled operations such as data compression and alert checking.
    """
    def __init__(self, db_manager: DatabaseManager, check_interval=60):
        self.db_manager = db_manager
        self.check_interval = check_interval  # in seconds
        self.running = False
        logger.debug("SchedulerManager initialized with check interval of %s seconds", check_interval)
    ...
    def clean_and_compress_reads(self):
        logger.debug("Compressing raw reads into ReadsClean...")
        sensors = self.db_manager.get_all_sensors()
        now = datetime.datetime.utcnow()
...
if __name__ == '__main__':
    ...
    scheduler = SchedulerManager(db_manager, check_interval=60)  # Check every 60 seconds
    scheduler.start()
    try:
        while True:
            time.sleep(5)
